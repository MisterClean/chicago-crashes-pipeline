...
class CrashValidator:
    ...
    def validate_crash_record(self, record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if record.get('latitude') and record.get('longitude'):
            ...
            if lat and (lat < self.validation_settings.min_latitude or 
                       lat > self.validation_settings.max_latitude):
                results['warnings'].append(f"Latitude {lat} outside Chicago bounds")
            
            if lon and (lon < self.validation_settings.min_longitude or 
                       lon > self.validation_settings.max_longitude):
