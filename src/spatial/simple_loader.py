...
class SimpleShapefileLoader:
    ...
    def load_all_shapefiles(
        ...
        if not shapefiles:
            ...
            return {
                ...
                "shapefiles_scanned": str(shapefiles_path),
            }
    ...
    def _load_single_shapefile(
        ...
        gdf.columns = [
            col.lower().replace(" ", "_").replace("-", "_") for col in gdf.columns
        ]
                ...
                for i, value in enumerate(row):
                    ...
                    if column_name.lower() in ["geometry", "geom", "the_geom"]:
                        row_dict[column_name] = (
                            f"<geometry: {str(value)[:50]}...>" if value else None
                        )
                    else:
...
def main():
    ...
    parser.add_argument(
        "--dir", default="data/shapefiles", help="Directory containing shapefiles"
    )
    parser.add_argument(
        "--list", action="store_true", help="List loaded spatial tables"
    )
        ...
        if args.list:
            ...
            if "error" in result:
                ...
                for table in result["tables"]:
                    ...
                    print(
                        f"  Geometry: {table['geometry_type']} (SRID: {table['srid']})"
                    )
                    print()
