...
class JobScheduler:
        ...
        try:
            ...
            for job in due_jobs:
                try:
                    ...
                    logger.error(f"Failed to execute job {job.name} (ID: {job.id}): {str(e)}")
    ...
    async def _execute_scheduled_job(self, job: ScheduledJob):
        ...
        try:
            ...
            running_executions = self.job_service.get_job_executions(job_id=job.id, limit=1)
            ...
            logger.info(f"Started execution {execution_id} for scheduled job {job.name}")
