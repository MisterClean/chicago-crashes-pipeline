...
class DataSanitizer:
    ...
    def sanitize_crash_record(self, record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Parse crash date
        sanitized["crash_date"] = self._parse_datetime(record.get("crash_date"))

        # Handle coordinates
        sanitized["latitude"] = self._clean_coordinate(
            record.get("latitude"), coord_type="latitude"
        )
        sanitized["longitude"] = self._clean_coordinate(
            record.get("longitude"), coord_type="longitude"
        )
    ...
    def sanitize_vehicle_record(self, record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        sanitized["crash_record_id"] = self._clean_string(record.get("crash_record_id"))
        sanitized["unit_no"] = self._clean_string(record.get("unit_no"))

        # Vehicle year validation
        sanitized["vehicle_year"] = self._clean_vehicle_year(record.get("vehicle_year"))
    ...
    def sanitize_fatality_record(self, record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        sanitized["rd_no"] = self._clean_string(record.get("rd_no"))
    ...
    def _clean_coordinate(self, value: Any, coord_type: str) -> Optional[float]:
        ...
        if coord_type == "latitude":
            if not (
                self.validation_settings.min_latitude
                <= coord
            ...
            if not (
                self.validation_settings.min_longitude
                <= coord
    ...
    def _clean_age(self, value: Any) -> Optional[int]:
        ...
        # Validate age range
        if not (
            self.validation_settings.min_age <= age <= self.validation_settings.max_age
        ):
            logger.debug(
                "Age out of valid range",
                age=age,
                range=(
                    self.validation_settings.min_age,
    ...
    def _clean_vehicle_year(self, value: Any) -> Optional[int]:
        ...
        # Validate year range
        if not (
            self.validation_settings.min_vehicle_year
            <= year
            <= self.validation_settings.max_vehicle_year
        ):
            logger.debug(
                "Vehicle year out of valid range",
                year=year,
                range=(
                    self.validation_settings.min_vehicle_year,
                    self.validation_settings.max_vehicle_year,
