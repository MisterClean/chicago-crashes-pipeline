...
## Option 1: Railway (Recommended for Small Projects)

[Railway](https://railway.app) is a container-based PaaS that makes deployment simple.
...
│                                                              │
│   ┌──────────────┐  ┌──────────────┐  ┌──────────────────┐  │
│   │   Frontend   │  │   Backend    │  │   PostGIS 17     │  │
│   │   (Next.js)  │──│   (FastAPI)  │──│   (PostgreSQL)   │  │
│   │              │  │              │  │                  │  │
│   │ Nixpacks     │  │ Dockerfile   │  │ Railway Template │  │
│   └──────────────┘  └──────────────┘  └──────────────────┘  │
│         │                  │                   │            │
...
1. **Create Railway Project**: Sign up at railway.app and create a new project

2. **Deploy PostGIS Database**:
   - Use Railway's PostGIS 17 template
   - Configure persistent volume for data durability

3. **Deploy Backend**:
   - Connect your GitHub repo
   - Set root directory to repo root
   - Configure `railway.toml` (see Configuration Files below)

4. **Deploy Frontend**:
   - Add another service from the same repo
   - Set root directory to `frontend/`
   - Nixpacks will auto-detect Next.js
...
|----------|-------------|
| `DB_HOST` | PostGIS internal hostname |
| `DB_PORT` | PostgreSQL port (usually 5432) |
| `DB_DATABASE` | Database name |
| `DB_USERNAME` | Database user |
| `DB_PASSWORD` | Database password (use Railway's secret management) |
| `ENVIRONMENT` | Set to `production` for security checks |
...
| Variable | Description |
|----------|-------------|
| `BACKEND_URL` | Backend internal URL for SSR |
| `NEXT_PUBLIC_API_URL` | Backend public URL for client-side |
| `NEXT_PUBLIC_PROTOMAPS_KEY` | Protomaps API key for basemap tiles |

### Configuration Files

#### Backend (`railway.toml` in repo root)
...
[deploy]
startCommand = "sh -c 'uvicorn src.api.main:app --host 0.0.0.0 --port ${PORT:-8000}'"
...
```bash
# Set environment variables for your Railway database
...
## Option 2: Docker Compose (Small Teams / Demos)
...
## Option 3: Container Orchestrator (Kubernetes, ECS, Nomad)
